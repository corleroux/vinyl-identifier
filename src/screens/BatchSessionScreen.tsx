...
export function BatchSessionScreen() {
  ...
  async function handleSaveAll() {
    const recordsWithNotes = batchQueue.map((r) => ({
      ...r,
      notes: notes[r.id] || r.notes || '',
    }))
    await db.records.bulkAdd(recordsWithNotes)
    clearBatch()
    navigate('/library')
  }
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <div className="flex-1 overflow-y-auto p-6 pb-32">
        <h1 className="text-2xl font-bold mb-6">{t('library.batchReview')}</h1>

        {batchQueue.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500 mb-4">{t('library.batchEmpty')}</p>
            <button
            ...
            {batchQueue.map((record) => (
              <div key={record.id} className="border rounded-lg p-4">
                ...
                <textarea
                  ...
                  className="mt-2 w-full text-sm border rounded px-3 py-2 resize-none"
                  rows={2}
                />
              </div>
            ))}
      ...
      {batchQueue.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t flex gap-3">
          <button onClick={handleSaveAll} className="btn-primary">
            {t('library.saveAll')} ({batchQueue.length})
          </button>
